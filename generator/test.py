...
def generateRandomPDS(number_of_states, alphabet_size, number_of_rules, number_of_smrules):
    global OFFSET
    OFFSET = 0
    ...
    while (len(rules) < number_of_rules):
        ...
        for i in range(num_branches):
            ...
            if rule_typ == 1:
                ...
                rule_name = 'r{}'.format(OFFSET)
                OFFSET += 1
                rules[rule_name] = (src, top, to_state, '{} {}'.format(new_top, ret), 'call')
            ...
            if rule_typ == 3:
                ...
                rule_name = 'r{}'.format(OFFSET)
                OFFSET += 1
                smrules[rule_name] = (src, ('', ''), to_state,)
...
def test_equivalence(state_size, alphabet_size, num_rules, num_smrules, caret_size, trial):
    ...
    res_vals = set()
    with subprocess.Popen(["./bcaret_mc_smpds", ctl_json_name, "-pytest", "-naive"], text=True, stdin=subprocess.PIPE, stderr=subprocess.PIPE, stdout=subprocess.PIPE) as proc1:
        max_mem = 0
        stdout = ''
        killed = False
        start_time = time.time()
